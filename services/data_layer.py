"""Data layer — PostgreSQL via SQLAlchemy. All queries scoped by org_id."""
...
class DataLayer:
    """Data access layer. All operations scoped to org_id."""
    ...
    def _require_org(self):
        """Raise 400 if no org_id is set — prevents unscoped cross-tenant queries."""
    ...
    async def create_org(self, name: str, domain: str = "") -> dict:
        ...
        org = Organization(name=name, domain=domain or None)
        self.db.add(org)
        await self.db.flush()
    ...
    async def list_signals(self, limit: int = 200) -> list[dict]:
        # Select only columns needed for display — raw_data is large and only needed for processing
        query = (select(Signal.id, Signal.type, Signal.source, Signal.title,
                        Signal.body, Signal.url, Signal.prioritized, Signal.created_at,
                        Signal.times_used, Signal.times_spiked)
                 .order_by(Signal.created_at.desc()).limit(limit))
        if self.org_id is not None:
            query = query.where(Signal.org_id == self.org_id)
        result = await self.db.execute(query)
        return [{"id": r.id, "type": r.type.value, "source": r.source, "title": r.title,
                 "body": r.body, "url": r.url, "prioritized": bool(r.prioritized),
                 "times_used": r.times_used or 0, "times_spiked": r.times_spiked or 0,
    ...
    async def list_content(self, status: str | None = None, limit: int = 50,
        ...
        # Exclude body_raw (pre-humanizer draft, never used in list views) to reduce payload
        query = (select(Content.id, Content.org_id, Content.signal_id, Content.brief_id,
                        Content.story_id, Content.channel, Content.status, Content.headline,
                        Content.body, Content.author, Content.source_signal_ids,
                        Content.created_at, Content.approved_at, Content.published_at,
                        Content.scheduled_at, Content.post_id, Content.post_url)
        ...
        result = await self.db.execute(query)
        return [_serialize_content_row(r) for r in result.all()]
    ...
    async def set_account_setting(self, key: str, value: str):
        ...
        # NOTE: intentionally NOT calling _check_writable() — account settings are user-scoped, not org-scoped
        query = select(Setting).where(Setting.key == key, Setting.org_id.is_(None))
        result = await self.db.execute(query)
        existing = result.scalar_one_or_none()
        if existing:
    ...
    async def get_memory_context(self) -> dict:
        ...
        spiked anti-patterns, recent topics per channel, and data sources."""
        channels = ["linkedin", "x_thread", "blog", "release_email", "newsletter"]
        memory = {"approved": {}, "spiked": {}, "recent_topics": [], "datasources": []}
        for ch in channels:
...
def _serialize_content_row(r) -> dict:
    """Serialize a projected row from list_content (no body_raw)."""
    return {
        "id": r.id, "org_id": r.org_id, "signal_id": r.signal_id,
        "brief_id": r.brief_id, "story_id": r.story_id,
        "channel": r.channel.value if hasattr(r.channel, 'value') else r.channel,
        "status": r.status.value if hasattr(r.status, 'value') else r.status,
        "headline": r.headline, "body": r.body, "body_raw": "",
        "author": r.author,
        "created_at": r.created_at.isoformat() if r.created_at else None,
        "approved_at": r.approved_at.isoformat() if r.approved_at else None,
        "published_at": r.published_at.isoformat() if r.published_at else None,
        "scheduled_at": r.scheduled_at.isoformat() if r.scheduled_at else None,
        "source_signal_ids": r.source_signal_ids or "",
